...
-- for example we have these 2 tables, salary and demographics - when a new employee joins and salary table is updated then demographics table should also be updated
-- to reflect that demographics table has the information about name, employee id and age, gender birth etc.
...
-- so really when we get a new row or data is inserted into the salary table we want a trigger to update the demographics table 
...
BEGIN
-- we want to update our employee table
-- and set the employee id, first name, last name from salary table
...
-- Let's insert a payment into the salary table and see if it updates the demographics table

-- so let's put the values that we want to insert
INSERT INTO employee_salary (employee_id, first_name, last_name, occupation, salary, dept_id)
VALUES(13, 'Jean-Ralphio', 'Saperstein', 'Entertainment 720 CEO', 1000000, NULL);
-- now it was updated in the salary table and the trigger was triggered and updated the corresponding values in the demographics table
select * from employee_salary;
-- check the details in demographics table
select * from employee_demographics;
